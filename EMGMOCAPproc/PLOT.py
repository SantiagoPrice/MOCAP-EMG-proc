...
def EMG_plotmultip(parts,phase="holding",save=False,debug=False, Norm="manual", channels=[0,8,9,10], met="mean" ,style=None):
    muscles = ['(R) Sternocleidomastoid', '(L) Sternocleidomastoid','(R) Para spinal','(L) Para spinal']
    ...
    for m_n , muscle in enumerate(muscles):
            ...
            for  part_lab, part_conds in parts.items():
              ...
              #Filtering participants
                ...
                if part_lab=="P 10" or part_lab=="P 8":# or part_lab=="P 7" or  part_lab=="P 1" or part_lab=="P 2" or part_lab=="P 5"or part_lab=="P 2" 
                     continue
                if part_lab == "P 11":
                    chans=[48,56,57,58]
                else:
                ...
                if Norm =="manual":
                    ...
                    ref_EMG=[]
                    for cond in pcs:
                        abs_val=EMG.EMG_stats(cond,"full_cicle" , Norm=[None], channels=[chans[m_n]] , metric="max")[:,0,:]
            ...
            if style==None:
                ...
                for mt_name , mt_key in mot_types.items():
                    ...
                    for mt_ind , mt in enumerate(mots):
                        nmt=motions.index(mt)
                        ax = fig.add_subplot(len(muscles),len(mots),(m_n*len(mots)+mt_ind+1))
                ...
                angs=dict()
                angs.update({"r":[[[31,3],[28,5]],[[29,3],[28,3]]]})
                angs.update({"s":[[[-12,3],[-9,4]],[[36,3],[39,1]]]})
                angs.update({"l":[[[-13,2],[-12,2]],[[13,2],[13,2]]]})
                ...
                for mt_name , mt_key in mot_types.items():
                    ...
                    fig=plt.figure(f"EMG_{met}_{style} group for {mt_name}: {phase} phase {Norm} normalization ("+",".join(exc_part)+" are excluded)")
                    if m_n == 0:
                        fig.suptitle(f"EMG_{met} for {mt_name}",fontsize = "xx-large")
                        fig.tight_layout(pad=3, w_pad=0.5, h_pad=0.50) 
                        figs.append(fig)
                    if mt_key == "l":
                        aux= [mots[1],mots[-2]]
                    ...
                    for mt_ind , mt in enumerate(mots):
                        ...
                        abs_ang= "("+str(np.abs(int(mt[1:])))+r"$^{\circ}$"+")"
                        
                        #ax.set_title(muscle + " ; " + side[mt_ind] + " " + abs_ang,fontsize = 22)
                        if m_n == 0:
                            ax.set_title(side[mt_ind],fontsize = 44)
                            ticklabs= [r"{}".format(angs[mt_key][mt_ind][ind][0])+"$^{\circ}\pm$"+"{}".format(angs[mt_key][mt_ind][ind][1])+"$^{\circ}$" for ind in [0,-1]]
                            is_top=True
                            ...
                            ticklabs= [jcond_lb[ind] for ind in [0,-1]]
                            is_top=False
                            fsx=40
                            is_bottom=not is_top
                        else:
                        ...
                        plt.boxplot(means[[0,-1],nmt,:].T,labels=ticklabs)    
                ...
                for mt_name , mt_key in mot_types.items():
                    ...
                    for mt_ind , mt in enumerate(mots):
                        ...
                        nmt=motions.index(mt)
                        print(phs)
                        ax = fig.add_subplot(len(muscles),3,(m_n*3+phs.index(phase)+1))
                ...
                for mt_name , mt_key in mot_types.items():
                    ...
                    for mt_ind , mt in enumerate(mots):
                        ...
                        ax.plot(means[rng,nmt,:],label=legs)
                        if m_n == 0 and mt_ind==0:
                            ax.legend(loc="upper right")
                        

def EMG_plotmultip2(parts,phase="holding",save=False,debug=False, Norm="manual", channels=[0,8,9,10], met="mean" ,style=None , mGroup=None):
    muscles = ['(R) Sternocleidomastoid', '(L) Sternocleidomastoid','(R) Para spinal','(L) Para spinal']
    muscles = ['right STR', 'left STR','right SPL','left SPL']
    Nparts= len(parts)
